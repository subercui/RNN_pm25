...
def normalize(X_train, X_test):
    X_mean = X_train.mean(axis=0).mean(axis=1)[None,None,:]
    print 'X_mean =', X_mean,X_mean.shape
    X_train -= X_mean
    X_stdev = np.sqrt(X_train.var(axis=0)).mean(axis=1)[None,None,:]
    X_train /= X_stdev
    X_test -= X_mean
    X_test /= X_stdev
...
if __name__=='__main__':
    ...
    LSTMmodel=build_rnn(X_train.shape[-1] ,y_train.shape[-1], 40, 40,layer_type=LSTM)
    train(X_train, y_train, X_valid, y_valid, LSTMmodel, batch_size=1024)
...
class Model:
    ...
    def __init__(self, hidden_size, input_size, output_size, stack_size=1, celltype=RNN,steps=40):
        ...
        # add a classifier:
        self.model.layers.append(Layer(hidden_size, 10, activation = T.tanh))
        self.model.layers.append(Layer(10, output_size, activation = lambda x:x))
...
dataset='/data/pm25data/dataset/RNNDetachValid'+today.strftime('%Y%m%d')+'_t100p100.pkl.gz'
...
# SAVE MODEL #
##############
savedir='/data/pm25data/model/DetachValidModel'+today.strftime('%Y%m%d')+'.pkl.gz'

...
class RNNPm25Dataset(object):
    ...
    def generateinput(self):
            ...
            if os.path.exists(gfsdir+filename) and os.path.getsize(gfsdir+filename)>0:#判断文件是否存在
                ...
                for entry in ['tmp','rh','ugrd','vgrd','prate','tcdc']:#填1个step上6个数据
                    ...
                    for k in range(self.n_location):
                        inputs[0*self.n_location+k,(h+3)/3,cnt]=interp(temp.reshape((180*4+1,360*4)),self.lat_x[k],self.lon_y[k])
                ...
                for entry in ['tmp','rh','ugrd','vgrd','prate','tcdc']:#填6个数据
                    for k in range(self.n_location):
                        inputs[0*self.n_location+k,(h+3)/3,cnt]=inputs[0*self.n_location+k,(h+3)/3-1,cnt]
                        #cnt是用来找对应dim3 元素的格位置，（h+3)/3是对应dim2 step位置
                cnt=cnt+1
        ...
        for i in range(1,self.n_perpoint):#填上矩阵中，剩余trainning example的数据
            ...
            for k in range(self.n_location):
                inputs[i*self.n_location+k,0:self.steps-1,0:self.n_element-1]=inputs[(i-1)*self.n_location+k,1:self.steps,0:self.n_element-1]
        ...
        for h in range(-3,t_predict+3,3):
            ...
            if int(name) > 2015061324:
                if os.path.exists(pm25dir+name[0:8]+'/'+name+'.pkl.gz') and os.path.getsize(pm25dir+name[0:8]+'/'+name+'.pkl.gz')>0:#判断文件是否存在
                    ...
                    f.close()
                    for k in range(self.n_location):
                        inputs[0*self.n_location+k,(h+3)/3,self.n_element-1]=temp[self.cord_x[k],self.cord_y[k]]-pm25mean[int(name[8:10])][self.cord_x[k],self.cord_y[k]]
                    #cnt是用来找对应dim3 元素的格位置，（h+3)/3是对应dim2 step位置
                else:#用3小时之前的替换
                    for k in range(self.n_location):
                        inputs[0*self.n_location+k,(h+3)/3,self.n_element-1]=inputs[0*self.n_location+k,(h+3)/3-1,self.n_element-1]                
                cnt=cnt+1
            else:
            ...
            for h in [t_predict]:#最后新的1位数据要读文件
                ...
                if int(name) > 2015061324:
                    if os.path.exists(pm25dir+name[0:8]+'/'+name+'.pkl.gz') and os.path.getsize(pm25dir+name[0:8]+'/'+name+'.pkl.gz')>0:#判断文件是否存在
                        ...
                        f.close()
                        for k in range(self.n_location):
                            inputs[i*self.n_location+k,(h+3)/3,self.n_element-1]=temp[self.cord_x[k],self.cord_y[k]]-pm25mean[int(name[8:10])][self.cord_x[k],self.cord_y[k]]
                    else:#用3小时之前的替换
                        for k in range(self.n_location):
                            inputs[i*self.n_location+k,(h+3)/3,self.n_element-1]=inputs[i*self.n_location+k,(h+3)/3-1,self.n_element-1]
                else:
                    if os.path.exists(pm25dir+name+'.pkl.gz') and os.path.getsize(pm25dir+name+'.pkl.gz')>0:#判断文件是否存在
                        f = gzip.open(pm25dir+name+'.pkl.gz', 'rb')
                        print(name+'.pkl.gz')
...
if __name__ == '__main__':
    ...
    savefile(obj.input_data,savedir+'RNNDetachValid'+today.strftime('%Y%m%d')+'_t100p100.pkl.gz')
    ...
    print "saved at "+savedir+'RNNDetachValid'+today.strftime('%Y%m%d')+'_t100p100.pkl.gz'

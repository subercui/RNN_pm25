...
pm25meandir='/ldata/pm25data/pm25mean/mean'+(today-datetime.timedelta(days=29)).strftime('%Y%m%d')+'/'
...
class RNNPm25Dataset(object):
    ...
    ''' 
    def __init__(self,lon=np.hstack((np.array([116.3883,117.20,121.48,106.54,118.78,113.66]),110+7.5*np.random.rand(194))),lat=np.hstack((np.array([39.9289,39.13,31.22,29.59,32.04,34.76]),34+6*np.random.rand(194))),start='2015040108',stop='2015051008',steps=int(t_predict/3+8)):
        ...
        self.n_element=6+3+1+1#6gfs+3时间特征+1均值+1pm25
    ...
    def generateinput(self):
        ...
        for h in range(-21,t_predict+3,3):
            ...
            if int(name) > 2015061324:#新文件路径
                if os.path.exists(pm25dir+name[0:8]+'/'+name+'.pkl.gz') and os.path.getsize(pm25dir+name[0:8]+'/'+name+'.pkl.gz')>0:#判断文件是否存在
                    ...
                    f.close()
                    for k in range(self.n_location):
                        inputs[0*self.n_location+k,(h+21)/3,self.n_element-2]=pm25mean[int(name[8:10])][self.cord_x[k],self.cord_y[k]]
        ...
        for i in range(1,self.n_perpoint):#同时生成每个location之后的数据
            ...
            for k in range(self.n_location):
                inputs[i*self.n_location+k,0:self.steps-1,self.n_element-2]=inputs[(i-1)*self.n_location+k,1:self.steps,self.n_element-2]
            ...
            for h in [t_predict]:#最后新的1位数据要读文件
                ...
                if int(name) > 2015061324:
                    if os.path.exists(pm25dir+name[0:8]+'/'+name+'.pkl.gz') and os.path.getsize(pm25dir+name[0:8]+'/'+name+'.pkl.gz')>0:#判断文件是否存在
                        ...
                        f.close()
                        for k in range(self.n_location):
                            inputs[i*self.n_location+k,(h+21)/3,self.n_element-2]=pm25mean[int(name[8:10])][self.cord_x[k],self.cord_y[k]]
...
if __name__ == '__main__':
    #a,b=lonlat2mercator()
    start=(today-datetime.timedelta(days=126)).strftime('%Y%m%d')+'08'
    ...
    savefile(obj.input_data,savedir+'forXiaodaDataset'+today.strftime('%Y%m%d')+'_t100p100.pkl.gz')
    ...
    print (savedir+'forXiaodaDataset'+today.strftime('%Y%m%d')+'_t100p100.pkl.gz')

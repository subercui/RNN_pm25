...
class RNNPm25Dataset(object):
    ...
    ''' 
    def __init__(self,lon=np.hstack((np.array([116.3883,117.20,121.48,106.54,118.78,113.66]),110+7.5*np.random.rand(94))),lat=np.hstack((np.array([39.9289,39.13,31.22,29.59,32.04,34.76]),34+6*np.random.rand(94))),start='2015090302',stop='2015092902',steps=int(t_predict/3+2)):
                ...
                if os.path.exists(gfsdir+filename) and os.path.getsize(gfsdir+filename)>0:#判断文件是否存在
                    ...
                    for entry in ['tmp','rh','ugrd','vgrd','prate','tcdc']:#填1个step上6个数据
                        ...
                        for k in range(self.n_location):
                            inputs[i+k*self.n_perpoint,(h+3)/3,cnt]=interp(temp.reshape((180*4+1,360*4)),self.lat_x[k],self.lon_y[k])
                    ...
                    for entry in ['tmp','rh','ugrd','vgrd','prate','tcdc']:#填6个数据
                        for k in range(self.n_location):
                            inputs[i+k*self.n_perpoint,(h+3)/3,cnt]=inputs[i+k*self.n_perpoint,(h+3)/3-1,cnt]
                            #cnt是用来找对应dim3 元素的格位置，（h+3)/3是对应dim2 step位置
                    cnt=cnt+1
...
if __name__ == '__main__':
    ...
    savefile(obj.input_data,savedir+'RNNTrueTest'+today.strftime('%Y%m%d')+'0903-0929.pkl.gz')
    ...
    print "saved at "+savedir+'RNNTrueTest'+today.strftime('%Y%m%d')+'0903-0929.pkl.gz'
